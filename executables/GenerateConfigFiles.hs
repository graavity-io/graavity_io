...
toEntityConfig theMap entName (static, eph) = do
  (pk, prk) <- PCrypto.genKeyPair
  kp <- head <$> mkKeyPairs [ApiKeyPair (PrivBS $ exportEd25519SecretKey prk) (Just $ PubBS $ exportEd25519PubKey pk) Nothing (Just ED25519) Nothing]
  return EntityConfig
