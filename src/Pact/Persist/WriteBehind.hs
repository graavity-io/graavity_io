...
data CacheAccess c = CacheAccess {
  ...
  cacheRead :: forall k v . (PactDbKey k, ToJSON v, FromJSON v, Typeable v) => Table k -> k -> Persist c (Either () (Maybe v)),
  -- | Used to populate cache after miss, possibly creating table in cache.
  cachePopulate :: forall k v . (PactDbKey k, ToJSON v) => Table k -> k -> Maybe v -> Persist c ()
  }
...
-- | Queries union cache and backend results.
query' :: (PactDbKey k, FromJSON v, Typeable v) => Table k -> Maybe (KeyQuery k) -> Persist (WB c w) [(k,v)]
query' t kq s = do
  mv <- newEmptyMVar
...
readValue' :: (PactDbKey k, ToJSON v, FromJSON v, Typeable v) => Table k -> k -> Persist (WB c w) (Maybe v)
readValue' t k s = do
...
writeValue' :: (PactDbKey k) => Table k -> WriteType -> k -> BS.ByteString -> Persist (WB c w) ()
writeValue' t wt k v s = do
  (s',_) <- case (wt,isNewTableForTx t s) of
    (Write,_) -> return (s,Just v)
    (_,True) -> return (s,Just v)
    _ -> (\(x, (a :: Maybe Value)) -> (x, Just v)) <$> readValue' t k s -- throwaway read to refresh cache for insert or update
