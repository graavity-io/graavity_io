...
import Pact.Types.Gas (GasEnv(..), MilliGasLimit(..))
import Pact.Types.Persistence
import Pact.Types.Runtime (_eeRefStore, permissiveNamespacePolicy, MilliGas(..))
...
readPureCache :: (PactDbKey k, ToJSON v, FromJSON v, Typeable v) => Table k -> k -> Persist Pure.PureDb (Either () (Maybe v))
...
populatePureCache :: (PactDbKey k, ToJSON v) => Table k -> k -> Maybe v -> Persist Pure.PureDb ()
populatePureCache t k v = return . (,()) . doInsert Pure.temp . doInsert Pure.committed
  where
    doInsert dbLens = over (dbLens . Pure.tblType t . Pure.tbls) $ \ts -> case (M.lookup t ts,v) of
        ...
        (Nothing,Just pv) -> M.insert t (Pure.Tbl $ M.singleton k (BSL.toStrict $ encode pv)) ts -- new table w value
        (Just {},Nothing) -> ts -- known table but key is empty, noop
        (Just (Pure.Tbl pt),Just pv) -> M.insert t (Pure.Tbl $ M.insert k (BSL.toStrict $ encode pv) pt) ts -- known table w update
...
runDemoWB = do
  ...
  let gasLimit = (0 :: Int64) -- TODO implement
  let gasRate = (0 :: Integer) -- TODO implement
  let gasEnv = (GasEnv (MilliGasLimit $ MilliGas gasLimit) 0.0 (constGasModel (fromIntegral gasRate)))
  let doExec pc pd = do
        pc' <- either (throwIO . userError) (return . ParsedCode pc) $ undefined
                 --AP.parseOnly (unPactParser exprs) pc -- TODO: fix it later
        rs <- takeMVar rv
        evalEnv <- setupEvalEnv
                        pde
                        def
                        Transactional
                        (MsgData pd def (pactHash "") [] [])
                        rs
                        gasEnv
  ...
  print =<< doExec pcode (toLegacyJson pdata)
  print =<< doExec "(demo.read-all)" (toLegacyJson Null)
