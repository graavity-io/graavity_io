...
module graavity.History.Persistence
  ( createDB
...
import qualified graavity.Types.Command as K (CommandResult(..))
import graavity.Types
import graavity.Types.Private
import graavity.Types.Sqlite
...
hashToField h = SText $ Utf8 $ BSL.toStrict $ A.encode $ toJsonViaEncode h

crToField :: Pact.CommandResult Pact.Hash -> SType
crToField cr = SText $ Utf8 $ BSL.toStrict $ A.encode $ toJsonViaEncode cr
