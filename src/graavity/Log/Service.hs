...
import graavity.Types.Command (CmdLatencyMetrics(..))
import graavity.Log.Persistence
import graavity.Types.Log
import graavity.Types.Metric
import graavity.Log
import graavity.Log.Types
import graavity.Log.LogApi as X
...
updateEvidenceCache (ULCommitIdx (UpdateCommitIndex _ci ts)) = do
  tellgraavityToApplyLogEntries ts
...
tellgraavityToApplyLogEntries :: UTCTime -> LogThread ()
tellgraavityToApplyLogEntries aerTime = do
  ...
  case mUnappliedEntries' of
    Just unappliedEntries' -> do
      ...
      debug $ "informing graavity to apply up to: " ++ show appliedIndex'
