...
import graavity.Types.Command as K (CommandResult(..))

newtype PollResponses = PollResponses (HM.HashMap RequestKey K.CommandResult)
  deriving (Eq, Show, Generic)

instance ToJSON PollResponses where
  toJSON (PollResponses m) = object $ map ((fromText . Pact.requestKeyToB16Text) *** toJSON) $ HM.toList m
