...
import graavity.Types.Base (LogIndex, NodeId)
import graavity.Types.Private (PrivateCiphertext,PrivateResult(..))
...
instance FromJSON UTCTime where
  parseJSON = withText "UTCTime" $ \x -> do
    case TR.readMaybe $ unpack x of
      Just t -> return t
      _ -> fail "unable to parse UTCTime"
...
instance (ToJSON a) => ToJSON (ClusterChangeCommand a) where
    toJSON (ClusterChangeCommand payload uSigs hsh) =
        object [ "cmd" .= payload
               , "sigs" .= (toJsonViaEncode <$> uSigs)
               , "hash" .= toJsonViaEncode hsh
               ]
...
instance ToJSON PactContractResult where
  toJSON (PactContractResult phash pres plogIndex pmetrics) =
          object [ "hash" .= toJsonViaEncode phash
                  , "result" .= toJsonViaEncode pres
                  , "logIndex" .= plogIndex
                  , "latMetrics" .= pmetrics
                  ]
...
instance ToJSON CommandResult where
  toJSON (SmartContractResult pr) = toJSON pr
  toJSON (ConsensusChangeResult chash cres clogIndex clatMetrics)  =
          object [ "hash" .= toJsonViaEncode chash
                  , "ncrResult" .= cres
                  , "logIndex" .= clogIndex
                  , "latMetrics" .= clatMetrics
                  ]
  toJSON (PrivateCommandResult chash cres clogIndex clatMetrics)  =
          object [ "hash" .= toJsonViaEncode chash
                  , "ivResult" .= cres
                  , "logIndex" .= clogIndex
                  , "latMetrics" .= clatMetrics
