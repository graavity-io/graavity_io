...
import graavity.Types.Base (Alias(..))
import graavity.Types.Comms (Comms(..),initCommsNormal,readCommNormal,writeCommNormal)
import graavity.Types.Entity (EntityLocal,EntityRemote)
...
instance {-# OVERLAPPING #-} Encode a => ToJSON (PrivateResult (Pact.CommandResult a)) where
  toJSON (PrivateSuccess a) = object ["tag" .= ("PrivateSuccess" :: String), "content" .= toJsonViaEncode a]
  toJSON PrivatePrivate = object ["tag" .= ("PrivatePrivate" :: String)]
  toJSON (PrivateFailure x) = object ["tag" .= ("PrivateFailure" :: String), "content" .= x]
