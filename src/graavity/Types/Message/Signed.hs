...
module graavity.Types.Message.Signed
...
instance Eq Provenance where
  (==) NewMsg NewMsg = True
  (==) (ReceivedMsg _ po pt) (ReceivedMsg _ po2 pt2) = po == po2 && pt == pt2
  (==) _ _ = False

instance Ord Provenance where
  compare NewMsg NewMsg = EQ
  compare (ReceivedMsg _ po pt) (ReceivedMsg _ po2 pt2) = compare po po2 <> compare pt pt2
  compare NewMsg _ = GT
  compare _ _ = LT
...
pickKey !KeySet{..} sRpc@(SignedRPC !Digest{..} _) =
  case Map.lookup _digNodeId _ksCluster of
    ...
    Just !key
      | show key /= (show _digPubkey) -> Left $! "Public key in storage doesn't match digest's key for msg: "
                                     ++ show sRpc
      | otherwise -> Right $! key
{-# INLINE pickKey #-}
