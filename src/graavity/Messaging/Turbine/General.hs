...
module graavity.Messaging.Turbine.General
  ( generalTurbine
  ) where
...
import graavity.Types hiding (debugPrint)
import graavity.Types.PreProc (ProcessRequestChannel(..), ProcessRequest(..))
import graavity.Messaging.Turbine.Util

instance Ord Ed25519.Signature where
  compare (Ed25519.Sig s1) (Ed25519.Sig s2) = compare s1 s2
