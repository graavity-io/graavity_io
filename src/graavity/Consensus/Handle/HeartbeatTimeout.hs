...
module graavity.Consensus.Handle.HeartbeatTimeout
    (handle)
where
...
import qualified graavity.Config.TMVar as TMV
import graavity.Consensus.Handle.AppendEntries (clearLazyVoteAndInformCandidates)
import qualified graavity.Types.Sender as Sender (ServiceRequest(..), AEBroadcastControl(..))
import graavity.Consensus.Util
import qualified graavity.Types as KD
