...
module graavity.Consensus.Util
  ( resetElectionTimer
...
debug s = do
  when (not (null s)) $ do   
    ...
    role' <- use csNodeRole
    case role' of
      Leader -> liftIO $! dbg $! "[graavity|\ESC[0;34mLEADER\ESC[0m]: " ++ s
      Follower -> liftIO $! dbg $! "[graavity|\ESC[0;32mFOLLOWER\ESC[0m]: " ++ s
      Candidate -> liftIO $! dbg $! "[graavity|\ESC[1;33mCANDIDATE\ESC[0m]: " ++ s
