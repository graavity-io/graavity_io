...
module graavity.Consensus.Raft.Common where
  ...
  where
    go getPartial = do
      ...
      case getPartial bytes  of
        Get.Partial getNextPartial -> do
          print "coming before x"
          go getNextPartial
        x -> do
          print "coming to x"
          pure x
