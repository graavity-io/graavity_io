...
module graavity.Evidence.Service
  ( runEvidenceService
  , initEvidenceEnv
...
import graavity.Config.ClusterMembership
import graavity.Config.TMVar as Cfg
import graavity.ConfigChange as CC
import graavity.Event (pprintBeat)
import graavity.Evidence.Spec as X
import graavity.Types.Metric (Metric)
import graavity.Types.Base
import graavity.Types.Comms
import graavity.Types.Config
import graavity.Types.Dispatch (Dispatch)
import graavity.Types.Event (ResetLeaderNoFollowersTimeout(..))
import graavity.Types.Evidence (Evidence(..), PublishedEvidenceState(..), CommitCheckResult (..))
import graavity.Types.Message (AppendEntriesResponse(..))
import graavity.Util.Util (linkAsyncTrack, throwDiagnostics)
import qualified graavity.Log.Types as Log
import qualified graavity.Types.Log as Log
import qualified graavity.Types.Dispatch as Dispatch
...
runEvidenceProcessor = do
  ...
  case newEv of
    VerifiedAER aers -> do
      ...
      put newEs
      publishEvidence
      when (newEs ^. esResetLeaderNoFollowers) tellgraavityToResetLeaderNoFollowersTimeout
...
tellgraavityToResetLeaderNoFollowersTimeout :: EvidenceService EvidenceState ()
tellgraavityToResetLeaderNoFollowersTimeout = do
