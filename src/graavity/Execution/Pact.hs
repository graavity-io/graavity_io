...
module graavity.Execution.Pact
...
import Pact.JSON.Legacy.Value (_getLegacyValue, toLegacyJson)
...
import Pact.Types.Gas (GasEnv(..), GasLimit, gasLimitToMilliGasLimit)
import Pact.Types.Logger (logLog,Logger,Loggers,newLogger)
import Pact.Types.Persistence
import Pact.Types.Pretty
import Pact.Types.RPC
import qualified Pact.Types.Runtime as Pact (PublicKeyText)
...
import graavity.Util.Util (linkAsyncBoundTrack)
...
applyExec cmd (ExecMsg parsedCode edata) ks = do
  ...
  let gasEnv = (GasEnv (gasLimitToMilliGasLimit _peGasLimit) gasPrice (constGasModel (fromIntegral gasRate)))
  evalEnv <- liftIO $ setupEvalEnv _peDbEnv (Just (_elName $ _ecLocal $ _peEntity)) _peMode
  ...
  mapM_ (handlePactExec sigs (_getLegacyValue edata)) _erExec
  return $ T2
...
-- | TODO!! The old graavity pact model keeps the sigs around from the original exec in memory
...
handlePactExec :: S.Set Pact.PublicKeyText -> Value -> PactExec -> PactM p ()
...
doRollback PactEnv{..} ContMsg{..} executed = do
  ...
  unless done $ do
      when (executed == Just True) $ publishCont _cmPactId prevStep True (_getLegacyValue _cmData)
...
doResume PactEnv{..} ContMsg{..} EvalResult{..} PactExec{..} = do
  ...
  unless isLast $ do
      when (_peExecuted == Just True) $ publishCont _cmPactId nextStep False (_getLegacyValue _cmData)
...
publishCont pactTid step rollback cData = do
  ...
  when _ecSending $ do
    let me = _elName _ecLocal
        ...
        (rpc :: PactRPC Value) = Continuation $ ContMsg pactTid step rollback (toLegacyJson cData) Nothing
        signer = _ecSigner
    cmd <- liftIO $ mkCommandWithDynKeys [signer] [] addy nonce Nothing rpc
