...
import graavity.ConfigChange (mkConfigChangeApiReq)
import graavity.HTTP.ApiServer
import graavity.Types.Base hiding (printLatTime)
import graavity.Types.Command as K
import graavity.Types.Entity (EntityName)
import qualified graavity.Types.HTTP as K
import graavity.Types.Private (PrivateResult)

--- Overlappable
--
instance ToJSON Pact.PactResult where
  toJSON = toJsonViaEncode

instance ToJSON Pact.ApiKeyPair where
  toJSON = toJsonViaEncode
...
mkExec code mdata privMeta = do
  ...
  cmd <- liftIO $ Pact.mkCommandWithDynKeys
      someKps
      []
      privMeta
      (T.pack $ show rid)
      Nothing
      (Exec (ExecMsg (T.pack code) (toLegacyJson mdata)))
...
sendMultiple' templateCmd replCmd startCount nRepeats singleCmd = do
  ...
  case NE.nonEmpty cmds' of
    Nothing -> flushStrLn $ "Apps.graavity.Client.sendMultiple' - empty list of commands"
    Just ne -> do
...
pprintTable val = do
  ...
  let rendered = fmap (A.toHashMapText . fmap (BSL.unpack . encode)) os
...
handleCmd cmd reqStr = case cmd of
  ...
  Poll s -> parseRK s >>= void . handlePollCmds False . RequestKey . Hash . Short.toShort
  PollMetrics rk -> do
    s <- use server
    rccc <- RWS.asks _rcClientConfig
    let ep  = _ccEndpoints rccc
    ...
    forM_ sList $ \(s',_) -> do
      server .= s'
      flushStrLn $ "##############  " ++ s' ++ "  ##############"
      void $ handlePollCmds True $ RequestKey $ Hash $ Short.toShort rk'
    server .= s
  Exit -> return ()
