...
import Nuchain.ConfigChange (mkConfigChangeApiReq)
import Nuchain.HTTP.ApiServer
import Nuchain.Types.Base hiding (printLatTime)
import Nuchain.Types.Command as K
import Nuchain.Types.Entity (EntityName)
import qualified Nuchain.Types.HTTP as K
import Nuchain.Types.Private (PrivateResult)
...
sendMultiple' templateCmd replCmd startCount nRepeats singleCmd = do
  ...
  case NE.nonEmpty cmds' of
    Nothing -> flushStrLn $ "Apps.Nuchain.Client.sendMultiple' - empty list of commands"
    Just ne -> do
