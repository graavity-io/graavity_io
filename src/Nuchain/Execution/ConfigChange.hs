...
module Nuchain.Execution.ConfigChange
...
import Nuchain.Types.Crypto (Signer(..), valid)
import Nuchain.Types.Message.Signed (DeserializationError(..))
import Nuchain.Types.Command(CCPayload(..), ClusterChangeCommand(..), ProcessedClusterChg (..))
...
processClusterChange cmd =
  ...
  in if hash' /= (_cccHash cmd)
     then ProcClusterChgFail $! "Nuchain.Execution.ConfigChange - Hash Mismatch in cluster change: "
