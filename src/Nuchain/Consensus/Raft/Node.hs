...
module Nuchain.Consensus.Raft.Node where
import Protolude
...
import Nuchain.Consensus.Raft.Store
import Nuchain.Consensus.Raft.RaftExampleT
...
nodeMain storageState storageType nid nids = do
      ...
      let (host, port) = RS.nidToHostPort nid
...
persistentFilePath nid = do
  ...
  pure $ tmpDir </> toS (Char8.unpack nid) </> "persistent"

logsFilePath :: NodeId -> IO FilePath
logsFilePath nid = do
  tmpDir <- Directory.getTemporaryDirectory
  pure (tmpDir </> toS (Char8.unpack nid) </> "logs")
...
mkExampleDir nid = do
  tmpDir <- Directory.getTemporaryDirectory
  let nodeDir = tmpDir </> toS (Char8.unpack nid)
  pure nodeDir
