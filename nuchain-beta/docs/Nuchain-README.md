# Nuchain Blockchain Documentation
---

Nuchain Version: 1.1.x
...
* `rlwrap`: only used in `nuchainclient.sh` to enable Up-Arrow style history. Feel free to remove it from the script if you'd like to avoid installing it.
* `tmux == v2.0`: only used for the local demo script `<nuchain-directory>/bin/<OS-name>/start.sh`.
...
NB: The docker and script files for installing the Nuchain dependencies can be found in `<nuchain-directory>/setup`.
...
```
<nuchain-directory>$ tmux
<nuchain-directory>$ ./bin/osx/start.sh
```
...
```
<nuchain-directory>$ tmux
<nuchain-directory>$ ./bin/ubuntu-16.04/start.sh
```


# Nuchain server and client binaries

### `nuchainserver`

Launch a consensus server node.
On startup, `nuchainserver` will open connections on three ports as specified in the configuration file:
...
```
nuchainserver (-c|--config) [-d|--disablePersistence]
...
NB: there is a `zeromq` bug that may cause `nuchainserver` to fail to launch (segfault) ~1% of the time. Once running this is not an issue. If you encounter this problem, please relaunch.

### `nuchainclient` & `nuchainclient.sh`
...
```
nuchainclient (-c|--config)

Options:
  -c,--config               [Required] path to client yaml configuration file

Sample Usage (found in nuchainclient.sh):
  rlwrap -A bin/nuchainclient -c "conf/$(ls conf | grep -m 1 client)"
```
...
The ability to do either of these is a feature of Nuchain -- because commands must have a unique hash and are either (a) signed or (b) fully encrypted, they can be redirected without degrading the security model.

### Replay From Disk

On startup but before `nuchainserver` goes online, it will replay from origin each persisted transaction.
If you would like to start fresh, you will need to delete the SQLite DB's prior to startup.

### Core Count

By default `nuchainserver` is configured use as many cores as are available.
...
* To allow nuchain to use all available cores, do not specify core count (remove the `+RTS -N[count] -RTS` section, or just the `-N[cores]` if using other runtime settings.)

### Beta Limitations

Beta License instances of Nuchain are limited as follows:
...
For a version without any/all of these restrictions, please contact us at [info@nuchain.io](mailto:info@nuchain.io).

### AWS Marketplace Limitations
The AWS Marketplace listing of Nuchain is limited as follows:
...
`nuchainserver` and `nuchainclient` each require a configuration file.
`genconfs` is designed to assist you in quickly (re)generating these files.

It operates in 2 modes:

* `./genconfs` will create a set of config files for a localhost test of nuchain.
...
Interaction with the cluster is performed via the Nuchain REST API, exposed by each running node.
...
### The `nuchainclient` tool

Nuchain ships with `nuchainclient`, which is a command-line tool for interacting with the cluster via the REST API.
...
`par-batch TOTAL_CMD_CNT CMD_RATE_PER_SEC DELAY` works much like `batch` except that `nuchainclient` will evenly distributed the new commands across the entire cluster.
...
Each nuchain node, while running, will host a performance monitor at the URL `<nodeId.host>:<nodeId.port>/monitor`.

#### Sample Usage: Running Pact TodoMVC

This repo also bundles the [Pact TodoMVC](github.com/kadena-io/pact-todomvc). Each Nuchain node will host the frontend at `<nodeId.host>:<nodeId.port>/todomvc`. To initialized the `todomvc`:

```
$ cd <nuchain-directory>

# launch the cluster

$ ./bin/<OS-name>/nuchainclient.sh
...
The Ansible playbooks and scripts we use for testing Nuchain on AWS are now available as well, located in
`<nuchain-directory>/aws`. Refer to `<nuchain-directory>/docs/Ansible-README.md` for detailed instructions on
how to use these Ansible playbooks and scripts.

#### Sample Usage: Querying the Cluster for Server Metrics

Each `nuchainserver` hosts an instance of `ekg` (a performance monitoring tool) on `<node-host>:<node-port>+80`.
...
status)
  for i in `cat nuchainservers.privateIp`;
    do echo $i ;
      curl -sH "Accept: application/json" "$i:10080" | jq '.nuchain | {role: .node.role.val, commit_index: .consensus.commit_index.val, applied_index: .node.applied_index.val}' ;
    done
  exit 0
  ;;
...
Nuchain uses SQLite for caching & persisting various by default.
...
* `logDir`: what directory to use for writing the HTTP logs, the Nuchain logs, and the various SQLite databases.
* `enableDebug`: should the node write any logs

While this is pretty low level tuning, Nuchain nodes can be configured to use different concurrency backends.
